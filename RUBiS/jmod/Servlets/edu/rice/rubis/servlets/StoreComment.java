    ...
    throws IOException, ServletException
  {
    ...
    try
    {
      ...
      try
      {
		...
		CommentsQuery cq = new CommentsQuery(fromId, toId, itemId, rating, new Date(), comment);
        stmt = cq.getStatement(conn);
        stmt.executeUpdate();
        stmt.close();
      }
      ...
      {
        ResultSet urs;
		GetUsersRating gur = new GetUsersRating(toId.intValue());
        stmt = gur.getStatement(conn);
        ...
        if (urs.first())
        {
		  ...
		  UpdateUsersQuery uuq = new UpdateUsersQuery(userRating, toId.intValue());
          stmt = uuq.getStatement(conn);
          stmt.executeUpdate();
        }
      }
      catch (SQLException e)
