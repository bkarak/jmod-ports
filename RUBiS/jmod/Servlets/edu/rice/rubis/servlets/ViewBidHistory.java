 ...
 */
  ...
  private boolean listBids(Integer itemId, PreparedStatement stmt, Connection conn, ServletPrinter sp)
  {
    ...
    try
    {
	  SelectBidsQuery sbq = new SelectBidsQuery(itemId.intValue());
      stmt = sbq.getStatement(conn);
    ...
    try
    {
      do
      {
        ...
        try
        {
		  GetUsersID gui = new GetUsersID(userId);
          stmt = gui.getStatement(conn);
    ...
    throws IOException, ServletException
  {
    ...
    {
      conn = getConnection();
	  GetItemsQuery gid = new GetItemsQuery(itemId.intValue());
      stmt = gid.getStatement(itemId.intValue());
      rs = stmt.executeQuery();
    }
    catch (Exception e)
    {
