...
public class ViewUserInfo extends RubisHttpServlet
{
      ...
      Connection conn, ServletPrinter sp)
  {
    ...
    try
    {
      ...
      try
      {
		SelectCommentsQuery scq = new SelectCommentsQuery(userId.intValue());
        stmt = scq.getStatement(conn);
      ...
      do
      {
        ...
        try
        {
		  GetUsersID guid = new GetUsersID(authorId);
          authorStmt = guid.getStatement(conn);
      ...
      throws IOException, ServletException
  {
    ...
    {
      conn = getConnection();
	  GetUsersQuery guq = new GetUsersQuery(userId.intValue());
      stmt = guq.getStatement(conn);
	  rs = stmt.executeQuery();
    }
    catch (Exception e)
    {
