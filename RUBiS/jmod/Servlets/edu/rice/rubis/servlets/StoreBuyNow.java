 ...
 */
    ...
    throws IOException, ServletException
  {
    ...
    try
    {
	  ...
	  GetItemsQuery giq = new GetItemsQuery(itemId.intValue());
      stmt = giq.getStatement(conn);
      ...
      if (quantity == 0)
      {
		UpdateItems3 ui3 = new UpdateItems3(new Date(), quantity, itemId.intValue());
        stmt = ui3.getStatement(conn);
        stmt.executeUpdate();
        stmt.close();
      }
      else
      {
		UpdateItems4 ui4 = new UpdateItems4(quantity, itemId.intValue());
		stmnt = ui4.getStatement(conn);
        stmt.executeUpdate();
        stmt.close();
      }
    }
    ...
    try
    {
	  BuyNowQuery bnq = new BuyNowQuery(userId.intValue(), itemId.intValue(), qty, new Date());
      stmt = bnq.getStatement(conn);
      stmt.executeUpdate();
