 ...
 */
    ...
    throws IOException, ServletException
  {
    ...
    try
    {
	  ...
	  InsertBidsQuery ibq = new InsertBidsQuery(userId, 
												itemId,
												qty,
												bid,
												maxBid,
												new Date());
      stmt = ibq.getStatement(conn);
      stmt.executeUpdate();
      stmt.close();
      ...
      try
      {
        ...
        if (rs.first())
        {
          ...
          {
            oldMaxBid = bid;
			UpdateItems ui = new UpdateItems(maxBid, nbOfBids, itemId.intValue());
            update = ui.getStatement(conn);
            update.executeUpdate();
            update.close();
          }
          else
          {
			UpdateItems2 ui2 = new UpdateItems2(nbOfBids, itemId.intValue());
            update = ui2.getStatement(conn);
            update.executeUpdate();
            update.close();
          }
