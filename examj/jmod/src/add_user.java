        ...
        btn_add.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                try {
                    ...
                    String encpass = encryptor.encrypt(String.valueOf(txt_root_pass.getPassword()));
                    Class.forName("org.gjt.mm.mysql.Driver");
                    String url = "jdbc:mysql://" + host + "/mysql";
                    connection = DriverManager.getConnection(url, username, encryptor.decrypt(encpass));
                ...
                || String.valueOf(usrpassword).length() > 0) {
                    ////////////////////////////
                    try {
						InsertUserQuery iuq = new InsertUserQuery(name, surname, email, usrname, md5(String.valueOf(usrpassword)));
						myStatement = iuq.getStatement(connection);
                        int add_usr = myStatement.executeUpdate();
                        if (add_usr == 1) {
                            JOptionPane.showMessageDialog(win_add_user, "User have been successfully Added!");
                            txt_root_pass.setText("");
                            win_add_user.setVisible(false);
                        } else {
                            JOptionPane.showMessageDialog(win_add_user, "User already exists!");
                            txt_root_pass.setText("");
                        }
						myStatement.close();
                        connection.close();
